      ...
      if (endpoint_data->bInterfaceProtocol == HID_ITF_PROTOCOL_KEYBOARD) {
        if (transfer->data_buffer[2] == HID_KEY_NUM_LOCK) {
          ...
          for (int i = 0; i < 6; i++) {
            ...
            if (keycode != 0 && !was_in_last(keycode)) {
              // Newly pressed key, even if report ordering changed
              usbHost->onKeyboardKey(usbHost->getKeycodeToAscii(keycode, shift), keycode, shift);
            }
          }
